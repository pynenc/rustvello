...
This guide will help you get started with rustvello. It covers all the fundamental features and functionalities of the library, making it easy for new users to familiarize themselves with the basics, from initial installation and setup to core functionalities.
...
```{code-block} python
from rustvello import Config, ClientType
...
```{code-block} rust
use rustvello::config::{Config, ClientType};
...
```{code-block} rust
use rustvello_proto::MetricValue;
...
rustvello allows you to record events to a file and replay them later, which is useful for testing and debugging.
...
rustvello includes a command-line interface (CLI) tool for interacting with recordings.
