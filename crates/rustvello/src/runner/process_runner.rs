...
/// A runner that spawns a blocking thread per invocation for isolation.
///
/// Each task runs via `tokio::task::spawn_blocking` in a dedicated OS thread,
/// providing thread-level isolation from the async runtime. This is suitable
/// for CPU-bound or blocking tasks that should not occupy an async worker.
///
/// **Note:** True process-level isolation (via `std::process::Command`) is
/// planned for a future version. Currently this runner uses thread isolation.
///
...
pub struct SpawnBlockingRunner {
    runner_id: RunnerId,
    app_id: Arc<str>,
    config: AppConfig,
...
impl Clone for SpawnBlockingRunner {
    fn clone(&self) -> Self {
        Self {
...
impl std::fmt::Debug for SpawnBlockingRunner {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SpawnBlockingRunner")
...
impl SpawnBlockingRunner {
    ...
    /// Execute a single invocation in a separate OS thread (simulating process isolation).
    ///
    /// In the current implementation, we use `spawn_blocking` to run the task
    /// in a separate thread, which provides thread-level isolation. True process
    /// isolation (via `std::process::Command`) requires an external worker binary
...
impl Runner for SpawnBlockingRunner {
    ...
    async fn run(&self) -> RustvelloResult<()> {
        ...
        async {
            tracing::info!(
                "ProcessRunner starting (max_concurrent={}, app_id={}, pid={})",
                self.max_concurrent,
                self.app_id,
                std::process::id()
            );
            ...
            while !self.is_shutdown() {
                ...
                let worker_sb_ctx =
                    runner_ctx.new_child(worker_runner_id.to_string(), "ProcessWorker");
                if let Err(e) = self
                    .state_backend
                ...
                handles.spawn(async move {
                    ...
                    let result = runner
                        .execute_invocation(&inv_id, &w_id, &worker_ctx)
                        .instrument(inv_span)
                        .await;
            ...
            tracing::info!("ProcessRunner shutting down");
    ...
    async fn run_one(&self) -> RustvelloResult<bool> {
        ...
        let worker_sb_ctx = runner_ctx.new_child(worker_runner_id.to_string(), "ProcessWorker");
        if let Err(e) = self
            .state_backend
            .store_runner_context(&worker_sb_ctx)
            .await
...
mod tests {
    ...
    fn test_runner_cls() {
        let (runner, _, _, _) = make_runner();
        assert_eq!(runner.runner_cls(), "SpawnBlockingRunner");
    }
