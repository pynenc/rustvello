...
use super::{
    deserialize_record, serialize_record, Mongo3Orchestrator, AUTO_PURGE_COL, CC_COL,
    HEARTBEAT_COL, STATUS_COL, WAITERS_COL,
};
use crate::connection::mongo_err;

#[async_trait]
impl OrchestratorStatus for Mongo3Orchestrator {
    ...
    async fn set_invocation_status(
        ...
        let new_record = status_record_transition(Some(&current_record), status, runner_id)
            .map_err(|e| match e {
                rustvello_proto::status::StatusMachineError::Transition(t) => {
                    RustvelloError::InvalidStatusTransition {
                        invocation_id: invocation_id.clone(),
                        from_status: t.from.unwrap_or(current_record.status),
                        to_status: t.to,
                        allowed_statuses: t.allowed,
                    }
                }
                rustvello_proto::status::StatusMachineError::Ownership(o) => {
    ...
    async fn register_invocation_with_id(
        ...
        match col.insert_one(doc_to_insert, None).await {
            Ok(_) => {}
            Err(e) => {
    ...
    async fn increment_invocation_retries(
        ...
        let result = col
            .find_one(doc! { "_id": invocation_id.to_string() }, None)
            .await
            .map_err(mongo_err)?;
        match result {
    ...
    async fn remove_invocation(&self, invocation_id: &InvocationId) -> RustvelloResult<()> {
        ...
        db.collection::<mongodb::bson::Document>(STATUS_COL)
            .delete_one(doc! { "_id": &inv_str }, None)
            .await
            .map_err(mongo_err)?;
        // Remove from CC
        self.remove_from_concurrency_index(invocation_id).await?;
        // Remove waiters
        db.collection::<mongodb::bson::Document>(WAITERS_COL)
            .delete_one(doc! { "_id": &inv_str }, None)
            .await
            .map_err(mongo_err)?;
        // Remove retries
    ...
    async fn run_auto_purge(&self, max_age_secs: u64) -> RustvelloResult<Vec<InvocationId>> {
        ...
        for invocation_id in expired {
            col.delete_one(doc! { "_id": invocation_id.as_str() }, None)
                .await
                .map_err(mongo_err)?;
