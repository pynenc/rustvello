...
use super::{cc_pair_mongo_key, Mongo3Orchestrator, CC_COL, STATUS_COL, WAITERS_COL};
use crate::connection::mongo_err;

#[async_trait]
impl OrchestratorQuery for Mongo3Orchestrator {
    ...
    async fn get_invocation_ids_paginated(
        ...
        let find_opts = mongodb::options::FindOptions::builder()
            ...
            .build();
        let mut cursor = col.find(filter, Some(find_opts)).await.map_err(mongo_err)?;
        let mut result = Vec::new();
        use futures_util::StreamExt;
        while let Some(doc_result) = StreamExt::next(&mut cursor).await {
            let d = doc_result.map_err(mongo_err)?;
    ...
    async fn get_blocking_invocations(&self, max_num: usize) -> RustvelloResult<Vec<InvocationId>> {
        ...
        let mut cursor = col
            .find(doc! { "waiters": { "$exists": true, "$ne": [] } }, None)
            .await
            .map_err(mongo_err)?;
        ...
        let mut waiting_cursor = col
            .find(doc! { "waiters": { "$in": &candidate_bsons } }, None)
            .await
            .map_err(mongo_err)?;
