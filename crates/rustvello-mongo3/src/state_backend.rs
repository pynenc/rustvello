...
#[non_exhaustive]
pub struct Mongo3StateBackend {
    pool: Arc<MongoPool>,
}

impl Mongo3StateBackend {
...
#[async_trait]
impl StateBackendCore for Mongo3StateBackend {
...
#[async_trait]
impl StateBackendQuery for Mongo3StateBackend {
    ...
    async fn get_workflow_run_offset(
        ...
        if col.count_documents(filter, None).await.map_err(mongo_err)? == 0 {
            return Ok(None);
        }
        let filter = doc! { "workflow_type": workflow_type.to_string(), "_id": { "$gt": workflow_id.to_string() } };
        Ok(Some(
            col.count_documents(filter, None).await.map_err(mongo_err)? as usize,
        ))
    }
    ...
    async fn count_workflow_runs(&self, workflow_type: &TaskId) -> RustvelloResult<usize> {
        ...
        Ok(col.count_documents(filter, None).await.map_err(mongo_err)? as usize)
    }
...
#[async_trait]
impl StateBackendRunner for Mongo3StateBackend {
    ...
    async fn get_history_in_timerange(
        ...
        let opts = mongodb::options::FindOptions::builder()
            ...
            .build();
        let mut cursor = col.find(filter, Some(opts)).await.map_err(mongo_err)?;
        let mut result = Vec::new();
        use futures_util::StreamExt;
        while let Some(doc_result) = StreamExt::next(&mut cursor).await {
            let d = doc_result.map_err(mongo_err)?;
